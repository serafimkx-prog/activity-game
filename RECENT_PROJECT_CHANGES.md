...
- Все словари сейчас бесплатные: первые три открыты сразу, остальные три открываются после входа через Telegram.
...
## 15. Quality Foundation Для Публичного Сайта

- Верхняя навигация `Новая игра / Правила / Профиль` переведена на нативные `button` внутри `nav.nav-pill`, с `aria-current="page"` для активного раздела.
- В главную SPA добавлен skip-link к настройке игры и видимые `:focus-visible` состояния для клавиатурной навигации.
- На setup-экране добавлен короткий блок `Быстрый старт`, который объясняет минимальный сценарий партии без длинного SEO-текста в первом экране.
